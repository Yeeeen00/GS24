    ...
    public class GameGrid : MonoBehaviour
    {
        ...
        private bool FillStep()
        {
            ...
            for (int x = 0; x < xDim; x++)
            {
                ...
                // Check if the dictionary contains the PieceType.Normal key
                if (!_piecePrefabDict.ContainsKey(PieceType.Normal))
                {
                    // Handle the case where PieceType.Normal key is not found in the dictionary
                    Debug.LogError("PieceType.Normal prefab is missing from _piecePrefabDict.");
                    return false; // Or handle in a way appropriate to your application
                }
